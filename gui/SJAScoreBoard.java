package gui;

import actor.GameObject;
import actor.Animal;
import grid.Grid;
import grid.Location;
import java.awt.Dimension;
import java.awt.Font;
import java.awt.Graphics;
import java.awt.Graphics2D;
import java.awt.Color;
import java.awt.FontMetrics;
import java.util.ArrayList;
import javax.swing.JPanel;
import world.SJAWorld;

/**
 *
 * @authors Spock, Cox, Angle
 * updated Spring 2021 - Liz Laverty
 */
@SuppressWarnings("serial")
public class SJAScoreBoard extends JPanel
{
    private ArrayList<Animal> animals = new ArrayList<Animal>(); 
    private static int maxScore = 0;
    
    //sorting variables
    public static final int POINTS = 0;
    public static final int TOTAL_POINTS = 1;
    public static final int ROUNDS_WON = 2;
    public static final int NAME = 3;
    
    private int sortBy = POINTS;
    private boolean debugMode = false;
    private Grid<GameObject> theGrid; 
    
    @SuppressWarnings("unchecked")
	public <T> SJAScoreBoard(Grid<T> gr)
    {
        theGrid = (Grid<GameObject>) gr;
    }
    
    public void findAnimalsInGrid()
    {
        animals.clear();     
        ArrayList<Location> occupied = theGrid.getOccupiedLocations();
        
        for(Location loc : occupied)
        {
            if(theGrid.get(loc) instanceof Animal)
                animals.add((Animal)theGrid.get(loc));
        }
    }
   
    public static int X_OFFSET = 20;
    public static int ROW_SIZE = 55; 
    
    /**
     * Paint this component.
     * @param g the Graphics object to use to render this component
     */
    public void paintComponent(Graphics g)
    {
    	int fontSize = 12;
        Graphics2D g2 = (Graphics2D) g;
        
        findAnimalsInGrid();
        
        super.paintComponent(g2);
        
        g.setFont(g.getFont().deriveFont((float) fontSize));
        g.drawString("Round #"+ SJAWorld.getRoundNum()+" Move #"+ SJAWorld.getMoveNum(), X_OFFSET, 30);

        ArrayList<Animal> sorted = sortAnimals();
        
        for(int rb=0; rb<animals.size(); rb++)
        {
            Animal r = sorted.get(rb);
            int w = ROW_SIZE*2;
            int h = ROW_SIZE-5;

            g.setFont(new Font("Helvetica", Font.PLAIN, fontSize));
            
            //draw rectangle
            g.setColor(r.getColor());
            g.fillRect(X_OFFSET, 50 + ROW_SIZE*rb, 200, ROW_SIZE - 2);

            //write text info (score, RW, etc) to screen
            g.setColor(invertColor(r.getColor()));
            String roundPoints = String.valueOf(r.getScore());
            int averagePointsPerRound = (int)(r.getTotalScore()/Math.max(SJAWorld.getRoundNum()-1,1));
            String averagePoints = String.valueOf(averagePointsPerRound);
            String roundsWon = String.valueOf(r.getRoundsWon());
            //Round Points
            if(sortBy == POINTS) g.setFont(new Font("Helvetica", Font.PLAIN, fontSize));
            else g.setFont(new Font("Helvetica", Font.BOLD, fontSize));
            FontMetrics fm = g.getFontMetrics();
            g.drawString(roundPoints, X_OFFSET + 40 - fm.stringWidth(roundPoints),
                    50 + ROW_SIZE*rb + 28);
            g.setFont(new Font("Helvetica", Font.PLAIN, fontSize));
            fm = g.getFontMetrics();
            g.drawString("Score", X_OFFSET + 40 - fm.stringWidth("Score"), 
                    50 + ROW_SIZE*rb + h/2 + 20);
            
            //Total Points
            if(sortBy == TOTAL_POINTS) g.setFont(new Font("Helvetica", Font.PLAIN, fontSize));
            else g.setFont(new Font("Helvetica", Font.BOLD, fontSize));
            fm = g.getFontMetrics();
            g.drawString(averagePoints, X_OFFSET + 110 - fm.stringWidth(averagePoints),
                    50 + ROW_SIZE*rb + 28);
            g.setFont(new Font("Helvetica", Font.PLAIN, fontSize));
            fm = g.getFontMetrics();
            g.drawString("Average", X_OFFSET + 110 - fm.stringWidth("Average"), 
                    50 + ROW_SIZE*rb + h/2 + 20);
            
            //Rounds Won
            if(sortBy == ROUNDS_WON) g.setFont(new Font("Helvetica", Font.PLAIN, fontSize));
            else g.setFont(new Font("Helvetica", Font.BOLD, fontSize));
            fm = g.getFontMetrics();
            g.drawString(roundsWon, X_OFFSET + 160 - fm.stringWidth(roundsWon),
                    50 + ROW_SIZE*rb + 38);           
            
            g.setFont(new Font("Helvetica", Font.BOLD, fontSize+4));
            g.drawString(r.getAnimal().getName(),X_OFFSET + 40, 50 + ROW_SIZE*rb + 15);
                        
            if(debugMode)
            {
                g.setColor(Color.BLACK);
                g.drawString("Prev choice = "+r.getMostRecentChoice(), 
                        X_OFFSET + w + 55, 50 + ROW_SIZE*rb + 38);
                        }
        }
        calcMaxScore();
    }
    
    public void calcMaxScore() 
    {
        maxScore = -5000;
        for(Animal a : animals)
        {
            if(a.getScore() > maxScore)
                maxScore = a.getScore();
        }
    }
    
    public static int getMaxScore() { return maxScore; }
    
    /**
     * Returns the desired size of the display, for use by layout manager.
     * @return preferred size
     */
    public Dimension getPreferredSize()
    {
        return new Dimension(200,200);
    }

    /**
     * Returns the minimum size of the display, for use by layout manager.
     * @return minimum size
     */
    public Dimension getMinimumSize()
    {
        return new Dimension(200,200);
    }
    
    public void toggleDebugMode()
    {
        debugMode = !debugMode;
    }
    
    public void setSortOrder(int so){
        if(so <= 4 && so >= 0) sortBy = so;
        else sortBy = 0;
    }
    
    private ArrayList <Animal> sortAnimals(){
        ArrayList<Animal> sorted = new ArrayList<Animal>();
        if(animals.size() == 0) 
            return sorted;
        
        if(sortBy == NAME){
            ArrayList<String> origNames = new ArrayList<String>();
            for(int i = 0; i < animals.size(); i++) 
                origNames.add(animals.get(i).getAnimal().getName());
            
            ArrayList<String> sortedNames = sortAlphabetically(origNames);
            for(int i = 0; i < sortedNames.size(); i++){
                int n = origNames.indexOf(sortedNames.get(i));
                sorted.add(animals.get(n));
                origNames.remove(n);
                origNames.add(n, null);
            }
        } else {
            int val1 = 0;
            int val2 = 0;
            sorted.add(animals.get(0));
            
            for(int i = 1; i < animals.size(); i++){
                if(sortBy == POINTS){
                    val1 = animals.get(i).getScore();
                } if(sortBy == TOTAL_POINTS){
                    val1 = animals.get(i).getTotalScore();
                } if(sortBy == ROUNDS_WON){
                    val1 = animals.get(i).getRoundsWon();
                } 
                for(int k = 0; k < sorted.size(); k++){
                    if(sortBy == POINTS){
                        val2 = sorted.get(k).getScore();
                    } if(sortBy == TOTAL_POINTS){
                        val2 = sorted.get(k).getTotalScore();
                    } if(sortBy == ROUNDS_WON){
                        val2 = sorted.get(k).getRoundsWon();
                    } 
                    
                    if(val1 > val2){ 
                        sorted.add(k, animals.get(i));
                        break;
                    }
                }
                
                if(!sorted.contains(animals.get(i))) sorted.add(animals.get(i));
            }
        }
        
        return sorted;
    }
    
    private ArrayList<String> sortAlphabetically(ArrayList<String> s){
        ArrayList<String> sorted = new ArrayList<String>();
        sorted.add(s.get(0));
        for(int i = 1; i < s.size(); i++){
            for(int k = 0; k < sorted.size(); k++){
                if(sorted.get(k).compareTo(s.get(i)) > 0 || 
                        sorted.get(k).compareTo(s.get(i)) == 0){
                    sorted.add(k, s.get(i));
                    break;
                }
            }
            
            if(!sorted.contains(s.get(i))) sorted.add(s.get(i));
        }
        
        return sorted;
    }

    private Color invertColor(Color in)
    {
        float[] vals = new float[3];
        
        Color.RGBtoHSB(in.getRed(), in.getGreen(), in.getBlue(), vals);
        
        if(vals[2] < 0.85)
            return Color.WHITE;
            
        //otherwise return the inverse of the color.    
        return new Color(255-in.getRed(), 255-in.getGreen(), 255-in.getBlue());
    }
}
